...
class Training:
    ...
    def show_training_info(self) -> InfoMessage:
        ...
        return InfoMessage(type(self).__name__, self.duration, distance, speed, calories)
...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        return (self.CALORIES_MEAN_SPEED_MULTIPLIER * mean_spead + self.CALORIES_MEAN_SPEED_SHIFT) * self.weight / self.M_IN_KM * self.duration
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        return ((a * self.weight + (mean_spead ** power // self.height) * b * self.weight) * self.duration) 
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        a = 1.1
        b = 2
        return (mean_spead + a) * b * self.weight * self.duration 

def read_package(workout_type: str, data: list) -> Training:
    """Прочитать данные полученные от датчиков."""
    training_dictionary: dict = {'SWM': Swimming, 'RUN': Running, 'WLK': SportsWalking}
